...
import utils
import log
import Performance_scenarios as ps
    ...
    def run_fio(self):
        ...
        for name_l in range(len(self.name_list)):
            ...
            try:
                ...
                check = subprocess.getoutput(command_f,shell=True)
                ...
                utils.prt_log('', f"fio command failed or device does not exist", 2)
...
class Get_sys_info(object):
    ...
    def install_soft(self): 
        ...
        for i in range(len(command)):
            try:
                check = subprocess.getoutput(command[i],shell=True)
                check_code = check.check_returncode()
    ...
    def get_htop_info(self): 
        try:
            date_command = 'Date >> ./performance_data/sysinfo.txt'
            subprocess.run(date_command,shell=True)

            command1 = 'ls *.txt >> ./performance_data/sysinfo.txt'
            ...
            check_code = check.check_returncode()
            utils.write_log('', f"Write htop info to /performance_data/sysinfo.txt file , Done", 0)

            echo_command = 'echo " " >> ./performance_data/sysinfo.txt'
    ...
    def get_atop_info(self): 
        try:
            ...
            command1 = 'ls *.py >> ./performance_data/sysinfo.txt'
            # command1 = 'atop -r | head -60  >> ./performance_data/sysinfo.txt'
            check = subprocess.run(command1,shell=True)
            check_code = check.check_returncode()
            utils.write_log('', f"Write atop info to /performance_data/sysinfo.txt file , Done", 0)
    ...
    def get_top_info(self): 
        try:
            ...
            command1 = 'ls *.log >> ./performance_data/sysinfo.txt'
            # command1 = 'top  -n 55 -b -d 30 | head -55 >> ./performance_data/sysinfo.txt'
            check = subprocess.run(command1,shell=True)
            check_code = check.check_returncode()
            utils.write_log('', f"Write top info to /performance_data/sysinfo.txt file , Done", 0)
    ...
    def get_iostat_info(self): 
        try:
            ...
            command1 = 'pwd >> ./performance_data/sysinfo.txt '
            # command1 = 'iostat -x 1 5 /dev/* >> ./performance_data/sysinfo.txt'
            check = subprocess.run(command1,shell=True)
            check_code = check.check_returncode()
            utils.write_log('', f"Write iostat info to /performance_data/sysinfo.txt file , Done", 0)
