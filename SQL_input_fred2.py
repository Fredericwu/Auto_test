...
class Write_to_database(object):
    ...
    def SQL_text_input(self):
        ...
        try:
            ...
            query = f'''INSERT INTO {self.Text_Table_Name} (Key_ID, DRBD_type, Readwrite_type, blocksize, Number_of_Job, IOdepth, IOPS, MBPS) values (?,?,?,?,?,?,?,?)'''
            ...
            for data in self.list_data:      
                ...
                values = (self.Key_ID, DRBD_type, Readwrite_type, blocksize, Number_of_Job, IOdepth, IOPS, MBPS)
                cur.execute(query, values)
                sql_result = cur.execute(f'SELECT * FROM {self.Text_Table_Name}')
...
if __name__ == '__main__':
    pass
    # bbb = Handle_data_function()
    # ccc = Write_to_database()
    # Run_test_for_main()
